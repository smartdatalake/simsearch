...
public class JdbcConnector implements IDataConnector {
	...
	/**
	 * Provides a sample value from the specified column in a DBMS table.
	 ...
	 */
	public Object getSampleValue(String tableName, String colName) {

		Object val = null;
		try {	
			// Construct and execute an SQL query to get a NOT NULL value from the specified column of this table
			String sql = "SELECT " + colName + " FROM " + tableName + " WHERE " + colName + " IS NOT NULL LIMIT 1";
			val = findSingletonValue(sql); 
		} catch (Exception e) {
			e.printStackTrace();
		}
		return val;
	...
	public int getColumnNumber(String tableName, String colName) {
		...
		try {	
			...
			case "POSTGRESQL":   	// Connected to PostgreSQL
				sql = "SELECT ordinal_position FROM information_schema.columns WHERE table_name ='" + tableName + "' AND column_name = '" +  col + "'";
			...
			case "AVATICA":   		// Connected to Avatica JDBC (Proteus)
				...
				sql = "SELECT count(*) FROM (SELECT " + col + " FROM " + tableName + " WHERE " + col + " IS NOT NULL LIMIT 1) test";
	 ...
	 */
	public boolean isJDBCColumnIndexed(String tableName, String colName) {
		...
		try {		
			...
			switch(this.getDbSystem()) {
			case "POSTGRESQL":  // Connected to PostgreSQL
				sql = "SELECT i.relname AS index_name FROM pg_class t, pg_class i, pg_index ix, pg_attribute a WHERE t.oid = ix.indrelid AND i.oid = ix.indexrelid AND a.attrelid = t.oid AND a.attnum = ANY(ix.indkey) AND t.relkind = 'r' AND t.relname LIKE '" + tableName + "%' AND a.attname LIKE '" + colName + "%' ORDER BY t.relname, i.relname;";
				break;
			case "AVATICA":   // FIXME: Any alternative to examine if a specific column is indexed in Avatica JDBC connections (Proteus)?
				sql = "SELECT TRY_CAST(" + colName + " AS double) FROM " + tableName + " WHERE " + colName + " IS NOT NULL LIMIT 1";   // Work-around to identify queryable numerical attributes
				break;
			default:
				sql = null;
			} 
