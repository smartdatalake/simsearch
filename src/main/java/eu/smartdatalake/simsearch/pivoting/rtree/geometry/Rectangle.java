...
import eu.smartdatalake.simsearch.pivoting.rtree.geometry.internal.RectangleDouble;
...
//    	Preconditions.checkArgument(values.length >= 4 && values.length % 2 == 0, "must be at least 4 values passed and the number of values must be even");
        if (!(values.length >= 4 && values.length % 2 == 0))
    		throw new IllegalArgumentException("must be at least 4 values passed and the number of values must be even");
