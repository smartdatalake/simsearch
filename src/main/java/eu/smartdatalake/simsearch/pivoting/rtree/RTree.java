...
import eu.smartdatalake.simsearch.pivoting.rtree.internal.util.BoundedPriorityQueue;
 ...
 * Original R-tree code available here: https://github.com/davidmoten/rtree-multi
 * 
 * @param <T>
...
//          Preconditions.checkArgument(dimensions >= 2, "dimensions must be 2 or more");     	
        	if (dimensions < 2)
        		throw new IllegalArgumentException("dimensions must be 2 or more");
...
//        Preconditions.checkArgument(dimensions() == entry.geometry().dimensions(), entry + " has wrong number of dimensions, expected " + dimensions());
        if (dimensions() != entry.geometry().dimensions())
    		throw new IllegalArgumentException(entry + " has wrong number of dimensions, expected " + dimensions());
...
//          Leaf<T, S> node = context.factory().createLeaf(Lists.newArrayList((Entry<T, S>) entry), context);
        	List<Entry<T, S>> l = new ArrayList<Entry<T, S>>();
        	l.add((Entry<T, S>) entry);
        	Leaf<T, S> node = context.factory().createLeaf(l, context);
        	return new RTree<T, S>(node, size + 1, context);
        }
    }
    ...
    /***********************************************************************************/
    /**
     * Distance browsing algorithm to retrieve the k-nearest neighbors to the given point
     * @param p  query location
     * @param k  max number of entries to return
     * @return  the k nearest entries in ascending order of distance from p
     */
    public Iterable<NearestEntry<T, S, Double>> distanceBrowsing(final Point p, int k) {
    	
//    	System.out.println("Searching for k-NN...");
        
        if (root.isPresent())
            return DistanceBrowsing.search(root.get(), p, k);
        else
            return Collections.emptyList();
