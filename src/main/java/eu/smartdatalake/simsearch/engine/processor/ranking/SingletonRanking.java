...
public class SingletonRanking  implements IRankAggregator {
	 ...
	 */
	public SingletonRanking(Map<String, DatasetIdentifier> datasetIdentifiers, Map<String, ISimilarity> similarities, Map<String, Double[]> weights, Map<String, Thread> tasks, Map<String, RankedList> queues, Map<String, AtomicBoolean> runControl, int topk, Logger log) {
		...
		// Only one task should have been specified
		taskKey = tasks.keySet().stream().findFirst().get();
		
		// In case no weights have been specified, set to 1.0 for this single task (attribute)
		if (weights.get(taskKey) == null)
			weights.put(taskKey, new Double[] {1.0});
	...
	private boolean updateRankedList(String taskKey, int i) {
		...
		if (cand != null) {
			for (int w = 0; w < weightCombinations; w++) {
				...
				if (this.datasetIdentifiers.get(taskKey).getDatatype() == DataType.Type.DATE_TIME) //&& (!this.datasetIdentifiers.get(task).getDataSource().isInSitu()))
					attr.setValue(myAssistant.formatDateValue(cand.getValue()));  
